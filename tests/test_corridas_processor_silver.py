from etl import CorridasProcessorSilver
...
def spark():
    ...
    spark_session = SparkSession.builder \
        .master("local[*]") \
        .appName("CorridasProcessorSilverTests") \
...
def test_transform_data_filters_correctly(spark):
    sample_data = [
        ...
        Row(DATA_INICIO="02-01-2016 9:00", DATA_FIM="02-01-2016 09:30", CATEGORIA="Lazer",
            LOCAL_INICIO="C", LOCAL_FIM="D", DISTANCIA="20", PROPOSITO="Viagem"), # worng format data inicio filter
    ]
    raw_df = spark.createDataFrame(sample_data)

    processor = CorridasProcessorSilver("input", "output", "01-01-2016", "01-31-2016")
    transformed_df = processor.transform_data(raw_df)

    assert transformed_df.count() == 1


def test_transform_data_data_inicio_format_change(spark):
    sample_data = [
        Row(DATA_INICIO="01-15-2016 10:00", DATA_FIM="01-15-2016 10:30", CATEGORIA="Negócio",
            LOCAL_INICIO="A", LOCAL_FIM="B", DISTANCIA="10", PROPOSITO="Almoço"),
    ]
    ...
    assert "DATA_INICIO" in transformed_df.columns
    assert transformed_df.collect()[0].DATA_INICIO == "2016-01-15"

def test_transform_data_data_fim_format_change(spark):
    sample_data = [
        Row(DATA_INICIO="01-15-2016 10:00", DATA_FIM="01-15-2016 10:30", CATEGORIA="Negócio",
            LOCAL_INICIO="A", LOCAL_FIM="B", DISTANCIA="10", PROPOSITO="Almoço"),
    ]
    ...
    assert "DATA_FIM" in transformed_df.columns
    assert transformed_df.collect()[0].DATA_FIM == "2016-01-15"
